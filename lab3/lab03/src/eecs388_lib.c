...
char ser_read()
{
  /*
    ...
    1) wait until UART0 RX FIFO is not empty //ie read if something is there 
    2) read the data from the FIFO and return the read (one) byte.//read the data and return the data
    
    HINT: take a look at the implementation of ser_write()
  */
  uint32_t regval;
  //need to read in the rxdata, offset by 0x04 for rxdata
  //using 0x1001_3003 for UAPT 0 
