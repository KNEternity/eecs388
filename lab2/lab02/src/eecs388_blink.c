...
/*
   ...
   Task 1.4: Build and deploy/upload the code as is (DONE)
   
   Task 1.5: Try out debugging. See lab sheet. (DONE)
   ...
   Task 2.2: Implement RGB blinky. (DONE)
...
int main()
{
   int gpio1 = RED_LED; //GREEN_LED is a macro. It is essentially a placeholder that is replaced by the compiler during compilation into the correct pin number
                         //To see the actual pin numbers, look for GREEN_LED (or the other colors) in the eecs388_lib.h file
   int gpio2 = GREEN_LED; 
   int gpio3 = BLUE_LED; 
   int gpio4 = WHITE_LED; 
   gpio_mode(gpio1, OUTPUT); //this function sets the pin referred to by the variable gpio to act as an OUTPUT pin, i.e. send signals out of it
   ...
   while(1)
   {
      ...
      gpio_write(gpio1, ON); // the gpio_write() function sets a pin (referred to by gpio in this case) to the logic high or ON state.
      ...
      gpio_write(gpio1, OFF); // same as before, but this time we are setting the pin to logi low or OFF state. Same as turning the pin off
      delay(300);

      gpio_mode(gpio2, OUTPUT);
      gpio_write(gpio2, ON);
      delay(500);          // this function asks the processor to just do nothing and wait for 1000 milliseconds (1 sec)
      gpio_write(gpio2, OFF); // same as before, but this time we are setting the pin to logi low or OFF state. Same as turning the pin off
      delay(300);

      gpio_mode(gpio3, OUTPUT);
      gpio_write(gpio3, ON);
      delay(500);          // this function asks the processor to just do nothing and wait for 1000 milliseconds (1 sec)
      gpio_write(gpio3, OFF); // same as before, but this time we are setting the pin to logi low or OFF state. Same as turning the pin off
      delay(300);
